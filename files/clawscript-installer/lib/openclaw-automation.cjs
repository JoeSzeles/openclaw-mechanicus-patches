...
async function skillCall(skillName, args, options = {}) {
  console.log(`[openclaw-automation] skillCall skill="${skillName}" args=${JSON.stringify(args || null).slice(0, 80)}`);
  try {
...
async function fileExecute(filePath, args, options = {}) {
  console.log(`[openclaw-automation] fileExecute path="${filePath}" args=${JSON.stringify(args || null).slice(0, 80)}`);
  try {
