...
async function main() {
  ...
  for (var gc = 0; gc < gatewayChunks.length; gc++) {
    ...
    var INJECT_REPLACEMENT = "{ const __igApi = await __loadIgLocalApi(); if (__igApi && await __igApi.handleRequest(req, res)) return; }\nif (controlUiEnabled) {";
    if (gw.indexOf("mechanicus-ig-local-api") === -1 && gw.indexOf("mechanicus-ig-config-api") === -1 && gw.indexOf(INJECT_MARKER) !== -1) {
      var funcInsertPoint = gw.indexOf("async function handleRequest(req, res)");
      if (funcInsertPoint !== -1) {
        gw = gw.slice(0, funcInsertPoint) + IG_API_LOADER + "\n" + gw.slice(funcInsertPoint);
        gw = gw.replace(INJECT_MARKER, INJECT_REPLACEMENT);
        changed = true;
        console.log("[+] Injected ig-local-api.mjs loader into: " + path.basename(gatewayChunks[gc]));
      }
    }
    if (gw.indexOf("mechanicus-ig-config-api") !== -1 && gw.indexOf("mechanicus-ig-local-api") === -1) {
      gw = gw.replace(/\/\* mechanicus-ig-config-api \*\/[\s\S]*?return false;\n\}/, '/* mechanicus-ig-config-api removed — replaced by ig-local-api.mjs */');
      gw = gw.replace("if (__handleIgConfigApi(req, res)) return;\n", "");
      var funcInsertPoint2 = gw.indexOf("async function handleRequest(req, res)");
      if (funcInsertPoint2 !== -1) {
        gw = gw.slice(0, funcInsertPoint2) + IG_API_LOADER + "\n" + gw.slice(funcInsertPoint2);
        gw = gw.replace(INJECT_MARKER, INJECT_REPLACEMENT);
        changed = true;
        console.log("[+] Upgraded inline IG handler -> ig-local-api.mjs in: " + path.basename(gatewayChunks[gc]));
      }
    }
    if (changed) {
